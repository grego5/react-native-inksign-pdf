...
$pathRules = @(
    ...
    [pscustomobject]@{ Name = "native geometry"; Pattern = '^(core/ink-engine/upstream/|core/ink-engine/replay-tool/|core/ink-engine/modeling/|core/ink-engine/input/|core/ink-engine/primitives/StrokeOutline|core/ink-engine/InkEngine|core/ink-engine/tests/(replay|upstream)/)'; Checks = @("native_geometry", "geometry_contract", "diff_check"); Area = "native-geometry" },
    [pscustomobject]@{ Name = "native C++"; Pattern = '^(core/ink-engine/|core/pdfium-adapter/)'; Checks = @("native_lifecycle", "diff_check"); Area = "native-cpp" },
