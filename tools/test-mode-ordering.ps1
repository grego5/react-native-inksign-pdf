...
$iosPath = Join-Path $root "ios\InkSignView+Document.swift"
...
Assert-Contains $iosBody 'requireViewportReady\(request: request\)' 'iOS checks mode readiness'
Assert-Ordered $iosBody 'Self.parseViewport(viewport)' 'viewportRequestID &+= 1' 'iOS accepts after validation'
