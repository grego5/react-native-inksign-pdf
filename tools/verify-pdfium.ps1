...
$iosStaticBinaries = @(
  ...
  (Join-Path $iosXcframeworkRoot "ios-arm64-simulator\PDFium.framework\PDFium"),
  (Join-Path $iosXcframeworkRoot "ios-x86_64-simulator\PDFium.framework\PDFium")
)
...
if ($iosStaticInstalled) {
  foreach ($iosBinary in $iosStaticBinaries) {
    ...
    $iosMagic = [System.Text.Encoding]::ASCII.GetString((Get-ArchiveBytes $iosBinary)[0..7])
    if ($iosMagic -ne "!<arch>`n") {
      $iosStaticInstalled = $false
      break
    }
  }
...
$iosStatus = if ($iosStaticInstalled) { "static iOS XCFramework" } elseif ($missingPackagedLibraries.Count -eq $manifest.distribution.artifacts.Count) { "release-only iOS artifacts pending" } else { "experimental iOS slices pending" }
if ($missingPackagedLibraries.Count -gt 0) {
  Write-Output "PASS PDFium $($manifest.version): release assets configured; local packaged libraries are intentionally absent pending the first static release ($($missingPackagedLibraries -join ', ')); $iosStatus"
} else {
