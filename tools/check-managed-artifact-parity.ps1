...
$androidView = Read-Source "android/src/main/java/com/margelo/nitro/inksignpdf/HybridPdfView.kt"
...
$iosView = Read-Source "ios/PdfView.swift"
$iosDocument = Read-Source "ios/PdfView+Document.swift"
$iosExport = Read-Source "ios/PdfView+Export.swift"
...
Require $example 'react-native-fs|RNFS' 'example can retire picker copies'
Require $example 'useEffect' 'example cleans picker copies on unmount'
Require $example 'unlink' 'example retires exact picker paths'
Require $example 'cleanupPickerPath' 'example uses one picker-path cleanup helper'
Require $example 'const previousPath = selectedPdf\.current\?\.path' 'example captures the exact previous source'
Require $example 'cleanupPickerPath\(previousPath\)' 'example retires the previous source after replacement'
Require $example 'if \(pickerPath !== previousPath\) cleanupPickerPath\(pickerPath\)' 'example retires failed-open candidates'
