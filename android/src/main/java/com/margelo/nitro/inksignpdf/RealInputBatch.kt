...
/** Reusable packed page-space inputs for one MotionEvent real-input batch. */
internal class RealInputBatch {
  ...
  internal fun add(
    ...
    check(count < MAX_INPUTS) { "Real input batch is full" }
    buffer.putDouble(x)
    buffer.putDouble(y)
