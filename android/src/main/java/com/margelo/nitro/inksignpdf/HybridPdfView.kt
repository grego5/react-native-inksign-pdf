...
@DoNotStrip
class HybridPdfView internal constructor(
  ...
  override fun open(path: String, options: ViewportOptions?): Promise<PageInfo> {
    return launchPromise {
      ...
      val info = awaitWorkerResult(request.generation) { completion ->
        pdfWorker.replace(path, request.generation, completion)
      }
  ...
  override fun nextPage(): Promise<PageInfo> = navigatePage(1)

  override fun previousPage(): Promise<PageInfo> = navigatePage(-1)

  override fun getViewport(): Promise<Viewport> {
  ...
  override fun enterEditMode(viewport: ViewportOptions?): Promise<Unit> {
    return enterMode(edit = true, viewport)
  }

  override fun enterViewMode(viewport: ViewportOptions?): Promise<Unit> {
    return enterMode(edit = false, viewport)
  }

  override fun undo() {
    runHistoryCommand(surface::undo)
  }
  override fun redo() {
    runHistoryCommand(surface::redo)
  }
  override fun clear() {
    runHistoryCommand(surface::clear)
  }
  ...
  override fun insertAnnotationOn(): Promise<Unit> {
    val requestGeneration = synchronized(this) { documentGeneration }
    return launchPromise {
      checkMainThread()
      if (disposed || documentGeneration != requestGeneration) {
      ...
      if (textOverlay.hasPendingPlacement()) return@launchPromise Unit
      ...
      surface.withStateTransaction {
        ...
        if (disposed || documentGeneration != requestGeneration) throw operationCancelled()
        if (requestID != viewportRequestID) throw operationCancelled()
        textOverlay.armPlacement(requestGeneration)
      }
      Unit
    }
  }

  override fun insertAnnotationOff(): Promise<Unit> {
    val requestGeneration = synchronized(this) { documentGeneration }
    return launchPromise {
      checkMainThread()
      if (disposed || documentGeneration != requestGeneration) {
  ...
  override fun increaseTextSize(): Promise<Double> = runTextCommand {
    textOverlay.increaseTextSize()
  }

  override fun decreaseTextSize(): Promise<Double> = runTextCommand {
    textOverlay.decreaseTextSize()
  }

  override fun removeTextAnnotation(): Promise<Unit> = runTextCommand {
    textOverlay.removeTextAnnotation()
  }

  private fun navigatePage(delta: Int): Promise<PageInfo> {
    val requestGeneration = synchronized(this) { documentGeneration }
    return launchPromise {
      checkMainThread()
      if (disposed || documentGeneration != requestGeneration) {
      ...
      if (target == current.pageIndex) return@launchPromise toPublicPageInfo(current)

      var switched: PdfPageInfo? = null
      surface.withStateTransaction {
        textOverlay.finishForLifecycle()
        switched = surface.switchPage(target)
      }
      if (disposed || documentGeneration != requestGeneration) {
        throw operationCancelled()
      }
      val result = toPublicPageInfo(checkNotNull(switched))
      result
    }
  }
  ...
  override fun startDebugRecording() {
    mainScope.launch {
      if (!disposed) traceRecorder.start()
    }
  }

  override fun stopDebugRecording() {
    mainScope.launch {
      if (!disposed) traceRecorder.stop()
    }
  }
  ...
  private fun <T> launchPromise(operation: suspend () -> T): Promise<T> {
    ...
    mainScope.launch {
      try {
        ...
        synchronized(this@HybridPdfView) {
  ...
  private fun <T> runTextCommand(action: () -> T): Promise<T> {
    val requestGeneration = synchronized(this) { documentGeneration }
    return launchPromise {
      checkMainThread()
      if (disposed || documentGeneration != requestGeneration) {
  ...
  private fun enterMode(edit: Boolean, viewport: ViewportOptions?): Promise<Unit> {
    val requestGeneration = synchronized(this) { documentGeneration }
    return launchPromise {
      checkMainThread()
      if (disposed || documentGeneration != requestGeneration) {
