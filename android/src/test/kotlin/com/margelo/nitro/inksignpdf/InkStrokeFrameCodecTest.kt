...
class InkStrokeFrameCodecTest {
  @Test
  fun invalidContourOffsetsFailDuringDecode() {
    assertThrows(IllegalArgumentException::class.java) {
      InkStrokeFrameCodec.decode(frame(segmentCount = 1, contourCount = 1, recordStart = 1))
    }
  }
  ...
  fun invalidContourRangeFailsDuringDecode() {
    assertThrows(IllegalArgumentException::class.java) {
      InkStrokeFrameCodec.decode(frame(segmentCount = 1, contourCount = 1, recordCount = 2))
    }
  }
  ...
  fun negativeCountsFailDuringDecode() {
    assertThrows(IllegalArgumentException::class.java) {
      InkStrokeFrameCodec.decode(frame(segmentCount = -1L))
    }
  }
  ...
  fun reusableTargetReplacesPreviousContents() {
    val target = InkStrokeFrame()
    val first = InkStrokeFrameCodec.decode(
    ...
    InkStrokeFrameCodec.decode(frame(), target)
  ...
  fun repeatedDecodeReusesDecoderScratch() {
    val target = InkStrokeFrame()
    InkStrokeFrameCodec.decode(frame(segmentCount = 1, contourCount = 1), target)
    InkStrokeFrameCodec.decode(frame(segmentCount = 1, contourCount = 1), target)
  ...
  private fun frame(
    type: Int = InkStrokeFrameCodec.COMMITTED_TYPE,
    ...
    val capacity = InkStrokeFrameCodec.HEADER_BYTES +
    ...
    buffer.putInt(InkStrokeFrameCodec.VERSION)
    buffer.putInt(type)
    buffer.putInt(0)
    buffer.putLong(1L)
    buffer.putLong(0L)
    ...
    while (buffer.position() < InkStrokeFrameCodec.HEADER_BYTES) buffer.put(0)
