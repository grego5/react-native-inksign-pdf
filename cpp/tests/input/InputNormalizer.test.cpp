...
StrokeInput sample(StrokeEventType event,
                   double time,
                   double x,
                   double y,
                   double pressure = -1.0,
...
int main() {
  ...
  CHECK(lifecycle.update(sample(StrokeEventType::Move, 0.0, 0.0, 0.0),
                         output).code == InputStatusCode::NotInProgress);
  CHECK(lifecycle.begin(sample(StrokeEventType::Move, 0.0, 0.0, 0.0),
                        output).code == InputStatusCode::InvalidEvent);
  CHECK(!lifecycle.inProgress());
  CHECK(lifecycle.begin(sample(StrokeEventType::Down, 0.0, 0.0, 0.0),
                        output).ok());
  CHECK(lifecycle.begin(sample(StrokeEventType::Down, 0.1, 1.0, 0.0),
                        output).code == InputStatusCode::AlreadyInProgress);
  CHECK(lifecycle.end(sample(StrokeEventType::Move, 0.1, 1.0, 0.0),
                      output).code == InputStatusCode::InvalidEvent);
  CHECK(lifecycle.inProgress());
  CHECK(lifecycle.end(sample(StrokeEventType::Up, 0.1, 1.0, 0.0), output).ok());
  CHECK(!lifecycle.inProgress());

  InputNormalizer validation;
  CHECK(validation.begin(sample(StrokeEventType::Down, 10.0, 1.0, 2.0),
                         output).ok());
  CHECK(!output.stylus.pressure && !output.stylus.tilt &&
        !output.stylus.orientation);
  const StrokeInput duplicate =
      sample(StrokeEventType::Move, 10.1, 2.0, 3.0, 0.5, 0.7, 1.2);
  ...
  CHECK(validation.update(sample(StrokeEventType::Move, 10.05, 3.0, 3.0),
                          output).code == InputStatusCode::TimeWentBackwards);
  // Equal timestamps remain exact and are accepted when the sample differs.
  CHECK(validation.update(sample(StrokeEventType::Move, 10.1, 3.0, 3.0),
                          output).ok());
  CHECK(output.time == 10.1 && output.position.x == 3.0);
  // A gap has constant work here: it emits exactly the supplied endpoint.
  CHECK(validation.update(sample(StrokeEventType::Move, 1000000.0, 4.0, 3.0),
                          output).ok());
  CHECK(output.time == 1000000.0 && output.position.x == 4.0);

  const double infinity = std::numeric_limits<double>::infinity();
  CHECK(validation.update(sample(StrokeEventType::Move, 1000001.0, infinity,
                                 0.0), output).code ==
        InputStatusCode::InvalidValue);
  CHECK(validation.update(sample(StrokeEventType::Move, 1000001.0, 5.0, 0.0,
                                 -0.5), output).code ==
        InputStatusCode::InvalidValue);
  CHECK(validation.update(sample(StrokeEventType::Move, 1000001.0, 5.0, 0.0,
  ...
  CHECK(reset.begin(sample(StrokeEventType::Down, 0.0, 0.0, 0.0), output).ok());
  reset.cancel();
  CHECK(!reset.inProgress());
  CHECK(reset.update(sample(StrokeEventType::Move, 21.0, 21.0, 0.0),
                     output).code == InputStatusCode::NotInProgress);
  CHECK(reset.begin(sample(StrokeEventType::Down, 30.0, 0.0, 0.0), output).ok());
