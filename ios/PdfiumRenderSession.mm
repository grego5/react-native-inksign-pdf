...
#include "../cpp/pdfium/PdfiumDocumentSession.hpp"
...
struct PdfiumSessionState final {
  ...
  dispatch_queue_t queue() const {
    return (__bridge dispatch_queue_t)queueOwner;
  }
};
...
- (nullable instancetype)initWithData:(NSData *)data
  ...
  state->queueOwner = (__bridge id)queue;
...
- (CGSize)pageSizeForPage:(NSUInteger)pageIndex error:(NSError **)error {
  PdfiumSessionState *state = static_cast<PdfiumSessionState *>(_state);
  if (state == nullptr) {
  ...
  return CGSizeMake(metadata.width, metadata.height);
}
...
- (BOOL)renderPage:(NSUInteger)pageIndex
  ...
  void (^work)(void) = ^{
    ...
    request.clip = {clip.minX, clip.minY, clip.maxX, clip.maxY};
