...
extension PdfView {
  ...
  func switchPage(
    ...
    documentView.go(to: destination(for: CGPoint(x: target.geometry.mediaBox.width / 2,
                                                  y: target.geometry.mediaBox.height / 2),
                                      page: target.page))
    documentView.layoutIfNeeded()
    pageTurnLifecycle.pageSwitchStarted(switchID: requestID, targetPageIndex: pageIndex)
    finishPageSwitchIfReady(requestID: requestID)
  ...
  func finishPageSwitchIfReady(requestID: UInt64) {
    ...
    applyViewport(zoom: Double(targetZoom),
                  focus: CGPoint(x: state.activePage.geometry.mediaBox.width / 2,
                                 y: state.activePage.geometry.mediaBox.height / 2),
                  fitToPage: true)
    guard pendingPageSwitchID == requestID,
          documentView.currentPage === state.activePage.page,
    ...
    _ = pageTurnLifecycle.pageSwitchReady(switchID: requestID)
