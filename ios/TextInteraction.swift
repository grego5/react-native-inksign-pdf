...
final class InkSignPdfTextInteractionOverlay: UIView, UITextViewDelegate,
  ...
  internal func armPlacement(generation: UInt64) throws {
    ...
    guard let presentation = presentation() else { throw PdfView.TextError.notReady }
    guard presentation.generation == generation else { throw PdfView.TextError.cancelled }
  ...
  func increaseTextSize() throws {
    try changeSelectedFont(by: textFontSizeStep)
  }

  func decreaseTextSize() throws {
    try changeSelectedFont(by: -textFontSizeStep)
  }

  func removeTextAnnotation() throws {
    guard let id = selectedAnnotationID else { throw PdfView.TextError.notFocused }
    ...
    if case .editing(let state) = interactionState,
      ...
      closeEditor()
      clearSelection()
      throw PdfView.TextError.notFocused
    }
  ...
  private func changeSelectedFont(by delta: CGFloat) throws {
    guard let id = selectedAnnotationID else { throw PdfView.TextError.notFocused }
    guard presentation() != nil else { throw PdfView.TextError.notReady }
    if case .editing(let state) = interactionState, state.original == nil {
      let updated = min(max(state.fontSize + delta, minimumTextFontSize), maximumTextFontSize)
      guard updated != state.fontSize else { return }
      var next = state
    ...
    guard fontSize != annotation.fontSize else { return }
  ...
  private func layoutEditor() {
    ...
    let text = editor.text.isEmpty ? "M" : editor.text
