...
final class PdfViewLifecycleTests: XCTestCase {
  ...
  func testTextInteractionRejectsPlacementWithoutReadyPresentation() {
    ...
    XCTAssertThrowsError(try overlay.armPlacement(generation: 0)) { error in
      guard let textError = error as? PdfView.TextError,
  ...
  func testTextPlacementOnAndOffAreIdempotent() throws {
    ...
    _ = try fixture.view.insertAnnotationOn()
    _ = try fixture.view.insertAnnotationOn()
    XCTAssertTrue(overlay.hasPendingPlacement())
    XCTAssertNil(textEditor(in: overlay))

    _ = try fixture.view.insertAnnotationOff()
    _ = try fixture.view.insertAnnotationOff()
  ...
  func testPendingPageSwitchCancellationIsExactlyOnceAgainstDelayedCallback() throws {
    ...
    try fixture.view.switchPage(to: 1) { result in
      ...
      guard let viewportError = error as? PdfView.ViewportError else {
  ...
  func testDocumentReplacementCancelsPendingPageSwitchExactlyOnce() throws {
    ...
    try fixture.view.switchPage(to: 1) { result in
      ...
      guard case .failure(let error) = result,
            let viewportError = error as? PdfView.ViewportError else {
  ...
  private func makeFixture(
    ...
    view.documentView.pageOverlayViewProvider = view.overlayProvider
    ...
    view.documentView.go(to: PDFDestination(page: pages[activePageIndex], at: CGPoint(x: 150, y: 200)))
    view.documentView.scaleFactor = view.documentView.scaleFactorForSizeToFit
  ...
  private func beginSubthresholdSettlement(in view: PdfView) {
  ...
  @discardableResult
  private func beginPull(in view: PdfView) -> Bool {
