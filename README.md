...
Native PDF signing for React Native. The module displays a local PDF, supports
velocity-driven ink and text annotations, then exports a PDF with committed ink
and text annotations. It includes a custom stroke engine using Google Ink
algorithms and low-latency rendering for Android, plus a simple PencilKit
compatibility layer for iOS. No web support.
    ...
    <View style={styles.screen}>
      ...
      <View style={styles.toolbar}>
        ...
        <Button
          title="Draw"
          disabled={!page}
          onPress={() => void pdf.current?.enterEditMode()}
        />
        <Button
          title="View"
          disabled={!page}
          onPress={() => void pdf.current?.enterViewMode()}
        />
        <Button
          title="Place text"
          disabled={!page}
        ...
        <Button
          ...
          onPress={() => pdf.current?.undo()}
        />
        <Button
          title="Redo"
          disabled={!state.canRedo}
          onPress={() => pdf.current?.redo()}
        />
        <Button
          title="Clear"
          disabled={!state.isDirty}
          onPress={() => pdf.current?.clear()}
        />
        <Button
