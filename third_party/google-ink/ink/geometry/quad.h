...
class Quad {
 ...
 private:
  // Build-only portability patch: MSVC correctly rejects constexpr evaluation
  // of Angle::Normalized(), which is intentionally a runtime operation in the
  // upstream snapshot.
  Quad(Point center, float width, float height, Angle rotation, float skew) {
    center_ = center;
    skew_ = skew;
    width_ = width;
    height_ = height;
